...
unit_declared_value
: Specifiy a declared unit value in USD. Specifying the unit declared value is preferred over specifying only the entire order declared value so that an accurate amount can be computed for partial fulfillments. If a value is specified for an order item, then `"declaried_value_service": true` will need to be sent in with the $info object section.

unit_customs_value
: Specifiy a customs unit value in USD. Specifying the unit customs value is preferred over specifying only the entire order customs value so that an accurate amount can be computed for partial fulfillments.
{:.code-defs}
